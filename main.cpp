...
#include "include/common_helper/common_helper_cv.h"
#include "include/image_processor/image_processor.h"

#define WORK_DIR "/home/tahufik/yolov7/resources/"
#define DEFAULT_INPUT_IMAGE "/home/tahufik/yolov7/resources/coba2.mp4"
...
int main(int argc, char **argv)
{
    ...
    cv::VideoCapture cap(DEFAULT_INPUT_IMAGE); /* if cap is not opened, src is still image */
    ...
    {
        printf("Initialization Error Goblok\n");
        return -1;
    }
    else
    {
    ...
    for (frame_cnt = 0; cap.isOpened() || frame_cnt < LOOP_NUM_FOR_TIME_MEASUREMENT; frame_cnt++)
    {
        ...
        if (cap.isOpened())
        {
            ...
            if (CommonHelper::InputKeyCommand(cap))
                break;
        };
        ...
        if (frame_cnt > 0)
        { /* do not count the first process because it may include initialize process */
